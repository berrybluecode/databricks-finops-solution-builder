...
const solutionAssets=[
  {n:'CEMEA FinOps Recommendations App',scope:'Internal',ready:'Recommended',url:null,summary:'Logfood-backed recommendations for monthly waste and savings across compute, SQL, pipelines and serving.',updated:'Internal signal · Sep 2026',s:['Platform team','FinOps team','Data engineer','AI product owner'],i:['Current state','Hindsight','Optimization','Prevention','Value'],c:['Compute','SQL & BI','Pipelines','Model serving'],a:['Workspace / platform','Team','Job / query / endpoint','Organization'],e:['Production','Multi-workspace','Global / all'],x:['Visibility','Control','Optimize','Govern'],caveat:'Internal field asset; central home is still being finalized.'},
  {n:'FinLake — FinOps Lakehouse Demo',scope:'Internal',ready:'Preview / demo',url:null,summary:'Executive-facing FinOps lakehouse demo for consolidated cost visibility and planning.',updated:'Internal signal · May 2026',s:['FinOps team','CFO / Finance','CIO / CDO'],i:['Current state','Foresight','Value'],c:['All cost domains','Commercial commitments'],a:['Business unit','Organization'],e:['Multi-workspace','Region / cloud','Global / all'],x:['Visibility','Plan'],caveat:'Demo only; no reusable deployment repository surfaced.'},
  {n:'Genie Optimization FE Toolkit',scope:'Internal',ready:'Specialist',url:null,summary:'Field queries and playbook work for Genie performance, QPM analysis and cost optimization.',updated:'Internal signal · Feb 2026',s:['Platform team','Data engineer','AI product owner'],i:['Hindsight','Optimization','Prevention'],c:['SQL & BI','AI tokens & models'],a:['Product / use case','Workspace / platform','User'],e:['Production','Multi-workspace'],x:['Visibility','Optimize','Govern'],caveat:'Internal field repository and still evolving.'},
  {n:'Model Router FinOps Demo',scope:'Internal',ready:'Preview / demo',url:null,summary:'Cost and quality trade-offs between open and frontier models using AI Gateway and context routing.',updated:'Internal signal · Jul 2026',s:['AI product owner','CFO / Finance','CIO / CDO'],i:['Foresight','Optimization','Value'],c:['AI tokens & models','Model serving'],a:['Product / use case','Job / query / endpoint'],e:['Production','Region / cloud'],x:['Optimize','Plan'],caveat:'Concept demo; customize assumptions before customer use.'},
...
function renderSolutionMatches(){
  ...
  $('#scopeFilters').innerHTML=['All','Internal','Databricks public','Community'].map(x=>`<button class="${solutionScope===x?'active':''}" data-scope="${x}">${x}</button>`).join('');
  ...
  $('#solutionMatches').innerHTML=ranked.slice(0,12).map((r,index)=>{
    ...
    return `<article class="match-card">
      ...
      <div class="match-foot"><span>${a.updated}</span>${a.scope==='Internal'?'<span class="internal-contact">Contact your Databricks Account team</span>':`<a href="${a.url}" target="_blank" rel="noopener">Open solution ↗</a>`}</div>
    </article>`;
  }).join('');
}
